...
from probe_preprocess import ProbePreprocessor, bold, bcolored, writeline, writelist
...
def printProbeDatasetResults(trainRes, evalRes, evalProbeRes):
    ...
    printTierResults("AND Probe", evalProbeRes["probeAND"], "red")
    printTierResults("OR Probe", evalProbeRes["probeOR"], "red")
    printTierResults("MORE Probe", evalProbeRes["probeMORE"], "red")
    printTierResults("LESS Probe", evalProbeRes["probeLESS"], "red")
...
def writeProbePreds(preprocessor, evalRes, epoch = 0, batchNum = 0):
    preprocessor.writeProbePreds(evalRes["probeAND"], "probeAND", epoch, batchNum)
    preprocessor.writeProbePreds(evalRes["probeOR"], "probeOR", epoch, batchNum)
    preprocessor.writeProbePreds(evalRes["probeMORE"], "probeMORE", epoch, batchNum)
    preprocessor.writeProbePreds(evalRes["probeLESS"], "probeLESS", epoch, batchNum)
...
def runProbeEvaluation(sess, model, data, epoch, probelogfile, batchNum = 0, getAtt = None):
    ...
    if data is not None:
        ...
        res["probeOR"] = runProbeEpoch(sess, model, data["probeOR"], train = False, epoch = epoch, getAtt = getAtt)
        res["probeMORE"] = runProbeEpoch(sess, model, data["probeMORE"], train = False, epoch = epoch, getAtt = getAtt)
        res["probeLESS"] = runProbeEpoch(sess, model, data["probeLESS"], train = False, epoch = epoch, getAtt = getAtt)
